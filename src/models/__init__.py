#!/usr/bin/python
# -*- coding:utf-8 -*-
from src.models.CNN_1 import CNN as CNN_1d
from src.models.cnn_1d import cnn_features as cnn_features_1d
# from models.AdversarialNet import AdversarialNet
# from models.resnet18_1d import resnet18_features as resnet_features_1d
# from models.Resnet1d import resnet18 as resnet_1d
# from models.AdversarialNet import AdversarialNet_multi